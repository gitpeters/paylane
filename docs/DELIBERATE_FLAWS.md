...
> **Post 04 has shipped** — the ledger is now the source of truth. The rows it removed are struck
> through and marked ✅ below; the flaws they describe live on only in the `post-04-before` tag.

| Flaw | Why it's there | Post that removes it |
|---|---|---|
| ~~`accounts.balance` is a mutable `BIGINT` treated as the source of truth~~ | The naive model writes a balance column directly instead of deriving it from the ledger — the setup for showing why that is unrecoverable | ✅ **Done in Post 04** — `balance` is now a documented **cache** of the ledger; invariant (c) in `analysis/capacity.sql` checks it against the entry sum |
| No `@Version` / optimistic-lock column anywhere | Two concurrent balance updates must be able to lose an update on purpose, so the race is demonstrable | **A later concurrency post** (not Post 04) — optimistic locking on the account. Post 04 made `balance` a cache but deliberately did **not** add `@Version`, so the lost-update race is still demonstrable |
...
| ~~`ledger_entries` pairs DEBIT and CREDIT against the **same** `account_id`~~ | A real double-entry pair needs a counterparty account (provider settlement / suspense). `sum(DEBIT)=sum(CREDIT)` passes trivially because both legs hit the same account, so the balance check proves nothing yet | ✅ **Done in Post 04** — a charge now DEBITs the system `PROVIDER_SETTLEMENT` account and CREDITs `MERCHANT_AVAILABLE` (two accounts, `V3`); an append-only trigger enforces immutability |
| No charge-path-wide `@Transactional` (insert → provider → attempt → ledger → balance → status each autocommit on their own) | A mid-flow failure leaves partial state — the reverse case Topic 05 builds on. **Do not wrap the provider call in a transaction before then.** Post 04 wraps *only* the two ledger legs + the two balance-cache updates in one transaction (double-entry requires it); that is **not** the charge-path boundary | **Topic 05** — a transaction boundary around the provider call |
